...
amc_pms_holding_non_aprn_df = spark.sql(
    """
SELECT
 CONCAT(CLIENTID,'-', UPPER(REPLACE(IFNULL(AMFICODE,''), ' ', '')),'-',SCHEMEID,'-PMS') AS SOURCE_SYSTEM_ID,
 UPPER(REPLACE(IFNULL(AMFICODE,''), ' ', '')) as CHANNEL_PARTNER_PARENT,
 CASE WHEN PRODUCT_TYPE IS NOT NULL
      THEN IFNULL(CONCAT(upper(REPLACE(IFNULL(AMFICODE,''),' ','')) ,'-', PRODUCT_TYPE),'')
      ELSE ''
 END AS CHANNEL_PARTNER,
...
WHERE NOT (
    AMFICODE IS NOT NULL
    AND UPPER(TRIM(AMFICODE)) NOT LIKE 'INA%'
    AND REFCODE4 IS NOT NULL
    AND TRIM(REFCODE4) <> ''
)
"""
)
...
amc_pms_holding_aprn_df = spark.sql(
    """
SELECT
 CONCAT(CLIENTID,'-', UPPER(REPLACE(IFNULL(REFCODE4,''), ' ', '')),'-',SCHEMEID,'-PMS') AS SOURCE_SYSTEM_ID,
 UPPER(REPLACE(IFNULL(REFCODE4,''), ' ', '')) as CHANNEL_PARTNER_PARENT,
 CASE WHEN PRODUCT_TYPE IS NOT NULL
      THEN IFNULL(CONCAT(upper(REPLACE(IFNULL(REFCODE4,''),' ','')) ,'-', PRODUCT_TYPE),'')
      ELSE ''
 END AS CHANNEL_PARTNER,
 CONCAT(CLIENTID,'-PMS') AS FOLIO_ACCOUNT,
 SUBSTR(SCHEMENAME,1,79) AS PRODUCT_NAME,
