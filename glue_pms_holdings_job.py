...
GROUP BY cm.CLIENTID, sm.SCHEMEID, sm.SCHEMENAME, ppd.FROMDATE,
         im.AMFICODE, im.REFCODE4, im.INTERID, prd_cp_code.PRODUCT_TYPE
"""
)
...
WITH keyed_raw AS (
    SELECT
        *,
        CASE WHEN INTERID = 64 THEN 'DIRECT' ELSE AMFICODE END AS AMFI_EFFECTIVE,
        CASE
            WHEN INTERID = 64 THEN 'DIRECT'
...
keyed AS (
    SELECT
        *,
        REPLACE(UPPER(AMFI_EFFECTIVE),' ','') AS AMFI_NORM,
        REPLACE(UPPER(KEY_VALUE),' ','') AS KEY_NORM
    FROM keyed_raw
)
SELECT
...
WHERE KEY_SOURCE <> 'APRN'
  AND (AMFI_NORM LIKE 'IN%' OR AMFI_NORM LIKE 'ARN%' OR AMFI_NORM = 'DIRECT')
  AND KEY_NORM <> ''
"""
)
